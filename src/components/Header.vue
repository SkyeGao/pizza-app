<template>
  <header>
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
      ...
      <a href="/" class="navbar-brand">Pizza點餐系統</a>
      <ul class="navbar-nav">
        <!--<li><router-link tag="div" to="/" class="nav-link">主頁</router-link></li>tag可以修改router-link的标签-->
        <li><router-link :to="{name:'homeLink'}" class="nav-link">主頁</router-link></li>
        <li><router-link :to="{name:'menuLink'}" class="nav-link">菜單</router-link></li>
      ...
      <ul class="navbar-nav ml-auto">
        ...
        <li><router-link :to="{name:'registLink'}" v-show="!isLogin" class="nav-link">注册</router-link></li>
        <li class="nav-link" v-show="isLogin">{{currentName}}</li>
